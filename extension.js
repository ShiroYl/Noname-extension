game.import("extension",function(lib,game,ui,get,ai,_status){return {name:"真白原神V1",content:function(config,pack){
        ...
        lib.groupnature.fd='hydro';
        lib.group.push('fd');
        lib.translate['fd']='枫';
		lib.translate['fd2']='枫丹';
		lib.groupnature.fd='geo';
        lib.group.push('ly');
        lib.translate['ly']='璃';
		lib.translate['ly2']='璃月';
// 死亡语音
   game.N_playDieAudio=function(playerID){
    ...
    character:{
        character:{
			...
			furinaZB: ["female", "fd", 4, ["qifenZhiZB","qifenZhiZB_reset","qifenExtraDamage", "wanzhongkuanghuanZB"],["des:芙宁娜，魔神名芙卡洛斯，是水神“芙卡洛斯”的人类分身与扮演者。曾经统治着水之国枫丹，退位后，芙宁娜不再扮演水神，过上普通人的生活。她可以巧妙运用元素爆发，让她兼顾强大的攻击和生存恢复能力。"]],
			...
			xianyunZB: ["female", "ly", 3, ["zhaoqiheyun_effect","heyunhuanhuaZB"],["des:闲云，仙名留云借风真君，绝云仙众三眼五显仙人之一。闲云擅使机关奇术，心向红尘烟火，酷爱美食，擅长发明，精通仙家符箓。千年前曾在战争中追随岩王帝君，守护璃月。当时大多以人类姿态现身于世，果敢伶俐，受人敬仰。战后隐居于奥藏山，无拘无束，多以鹤形态现世。新搬进璃月，虽自称不擅交际，人缘却颇为广泛，人们认为闲云幽默，唠叨，热心，好相处。而她却认为自己不善言辞。为人谦和，身有傲骨。"]],
        },
        translate:{
            paimengZB:"派蒙",
            navilletteZB:"那维莱特",
...
heyunhuanhuaZB: {
    ...
    forced: true,
    filter: function(event, player) {
        return player.storage.heyunhuanhuaZB > 0 && event.card && event.card.name == 'sha';
    },
    content: function() {
        // 增加额外伤害
        trigger.num += Math.min(player.storage.heyunhuanhuaZB, 3);
        // 重置鹤云幻化层数
        player.storage.heyunhuanhuaZB = 0;
        player.unmarkSkill('heyunhuanhuaZB');
    },
    mark: true,
    marktext: "鹤",
    intro: {
        markcount: "鹤",
        content: function(storage) {
            return '当前鹤云幻化层数：' + (storage || 0);
        }
    },
    group: ["heyunhuanhuaZB_distance"],
    subSkill: {
        distance: {
            mod: {
                globalTo: function(from, to, distance) {
                    if (to.storage.heyunhuanhuaZB > 0) {
                        //return distance + 2;
						return distance + to.storage.heyunhuanhuaZB;
                    }
                    return distance;
                },
                maxHandcard: function(player, num) {
                    return num + (player.storage.heyunhuanhuaZB || 0);
                }
            }
        }
    },
    onremove: function(player) {
        if (player.storage.heyunhuanhuaZB) {
            player.storage.heyunhuanhuaZB = 0;
            player.unmarkSkill('heyunhuanhuaZB');
        }
    }
...
zhaoqiheyun_effect: {
    ...
    usable: 3,
    filter: function(event, player) {
        return !player.hasSkill('zhaoqiheyun_forbid');
    },
    content: function() {
        var player = this.player;
        if (typeof player.storage.heyunhuanhuaZB === 'undefined') {
            player.storage.heyunhuanhuaZB = 0;
        }
        console.log("技能 zhaoqiheyun_effect 使用前，当前层数: ", player.storage.heyunhuanhuaZB);
        player.storage.heyunhuanhuaZB++;
        if (player.storage.heyunhuanhuaZB > 9) player.storage.heyunhuanhuaZB = 9;
        player.markSkill('heyunhuanhuaZB');
        console.log("技能 zhaoqiheyun_effect 使用后，当前层数: ", player.storage.heyunhuanhuaZB);
        game.addVideo('storage', player, ['heyunhuanhuaZB', player.storage.heyunhuanhuaZB]);
        player.updateMarks();
        player.addTempSkill('heyunhuanhuaZB_temp', { player: 'phaseBefore' });
    },
    ai: {
        order: 1,
        result: {
    ...
    intro: {
        content: "出牌阶段限3次，使用后进入[鹤云幻化]状态，每次使用增加1点层数，每层[鹤云幻化]会让你远离其他角色（即+1距离），提高1点手牌上限，你的下一个杀会消耗当前层数造成额外伤害，最高不超过3点额外伤害。"
    }
},
...
heyunhuanhuaZB_temp: {
    ...
    forced: true,
    filter: function(event, player) {
        return event.card && event.card.name !== 'sha' && player.storage.heyunhuanhuaZB > 0;
    },
    content: function() {
        console.log("触发 heyunhuanhuaZB_temp，清除鹤云幻化状态");
        player.storage.heyunhuanhuaZB = 0;
        player.unmarkSkill('heyunhuanhuaZB');
        player.addTempSkill('zhaoqiheyun_forbid', 'phaseEnd'); // 禁止使用[朝起鹤云]到本回合结束
        player.removeSkill('heyunhuanhuaZB_temp');
    },
    onremove: function(player) {
        if (player.storage.heyunhuanhuaZB) {
            player.storage.heyunhuanhuaZB = 0;
...
zhaoqiheyun_forbid: {
    ...
    intro: {
        ...
        translate:{
            ...
            "wanzhongkuanghuanZB_info": "出牌阶段限制1次，凝聚狂欢之意，构筑水沫的舞台,为你恢复2点生命值，抽2张牌，并增加1点气氛值。",
			qifenZhiZB_reset: "独舞",
            ...
            "wanxianghuahuiZB_info": "出牌阶段限1次，使你攻击范围内的角色获得[血偿勒令]，并可指定1至2名角色，对其出[杀]。对处于[血偿勒令]状态的角色使用[杀]或[锦囊]牌造成伤害后，增加1点生命之契。[血偿勒令]持续到你的回合结束。",
			hongsizhiZB: "死宴",
			...
			zhaoqiheyun_effect: "朝起鹤云",
            "zhaoqiheyun_effect_info": "出牌阶段限3次，使用后进入[鹤云幻化]并增加1点层数，使用除[杀]以外的牌会立刻退出[鹤云幻化]，并暂时无法使用本技能，直到下个你的回合开始。",
			zhaoqiheyun_forbid: "仙鹤已下落",
            "zhaoqiheyun_forbid_info": "朝起鹤云技能冷却中",
			mujizhuxing: "暮集竹星",
            "mujizhuxing_info": "暮集竹星",
			heyunhuanhuaZB: "鹤云幻化",
            "heyunhuanhuaZB_info": "锁定技，每层[鹤云幻化]会暂时增加1个手牌上限，你的下一个[杀]在命中后会消耗当前层数造成额外伤害,最高不超过3点额外伤害。当处于[鹤云幻化]状态，其他角色与你计算距离时+N(基于[鹤云幻化层数)。[鹤云幻化]持续到你的下个回合开始前。",
        },
    },
    intro:"真白原神V1主扩展",
    author:"真白",
    ...
    forumURL:"",
    version:"1.0",
},files:{"character":["paimengZB.jpg","navilletteZB.jpg","furinaZB.jpg","clorindeZB.jpg","arlecchinoZB.jpg","xianyunZB.jpg"],"card":[],"skill":[],"audio":[]}}})
